...
package hamr.SW_Impl_Instance_FlyZones_FlyZonesDatabase
...
import hamr._
import hamr.SW.FlyZonesDatabase_thr_Impl_seL4Nix

object FlyZonesDatabase extends App {

  val FlyZonesDatabaseBridge : hamr.SW.FlyZonesDatabase_thr_Impl_Bridge = {
    val keep_in_zones = Port[Base_Types.Bits] (id = 0, name = "SW_Impl_Instance_FlyZones_FlyZonesDatabase_keep_in_zones", mode = DataOut)
    val keep_out_zones = Port[Base_Types.Bits] (id = 1, name = "SW_Impl_Instance_FlyZones_FlyZonesDatabase_keep_out_zones", mode = DataOut)
  ...
  def sendOutput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    if(keep_in_zones_port.nonEmpty) {
      FlyZonesDatabase_thr_Impl_seL4Nix.keep_in_zones_Send(keep_in_zones_port.get)
      keep_in_zones_port = noData
    }

    if(keep_out_zones_port.nonEmpty) {
      FlyZonesDatabase_thr_Impl_seL4Nix.keep_out_zones_Send(keep_out_zones_port.get)
  ...
  def initialiseArchitecture(): Unit = {
    val ad = ArchitectureDescription(
      components = MSZ (FlyZonesDatabaseBridge),
      connections = ISZ ()
    )
    Art.run(ad)
  }
