...
Unit hamr_SW_CASE_Filter_LST_thr_Impl_seL4Nix_filter_out_Send(STACK_FRAME 
  art_DataContent d) {
  sb_filter_out_enqueue(d);
}

// is_empty filter_in: In EventDataPort
B hamr_SW_CASE_Filter_LST_thr_Impl_seL4Nix_filter_in_IsEmpty(STACK_FRAME_ONLY) {
  return sb_filter_in_is_empty();
}

// receive filter_in: In EventDataPort union_art_DataContent
Unit hamr_SW_CASE_Filter_LST_thr_Impl_seL4Nix_filter_in_Receive(STACK_FRAME
...
void pre_init(void) {
  printf("Entering pre-init of CASE_Filter_LST_thr_Impl\n");
  ...
  hamr_SW_CASE_Filter_LST_thr_Impl_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  hamr_SW_CASE_Filter_LST_thr_Impl_adapter_initialiseEntryPoint(SF_LAST);

  printf("Leaving pre-init of CASE_Filter_LST_thr_Impl\n");
}
...
int run(void) {
  ...
  for(;;) {
    ...
    hamr_SW_CASE_Filter_LST_thr_Impl_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
