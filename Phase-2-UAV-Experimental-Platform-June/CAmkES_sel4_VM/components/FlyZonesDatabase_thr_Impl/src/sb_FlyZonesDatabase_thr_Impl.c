...
Unit hamr_SW_FlyZonesDatabase_thr_Impl_seL4Nix_keep_in_zones_Send(STACK_FRAME 
  art_DataContent d) {
  sb_keep_in_zones_write(d);
}

// send keep_out_zones: Out DataPort CMASI__Polygon_i
Unit hamr_SW_FlyZonesDatabase_thr_Impl_seL4Nix_keep_out_zones_Send(STACK_FRAME 
  art_DataContent d) {
...
void pre_init(void) {
  ...
  hamr_SW_FlyZonesDatabase_thr_Impl_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  hamr_SW_FlyZonesDatabase_thr_Impl_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    hamr_SW_FlyZonesDatabase_thr_Impl_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
