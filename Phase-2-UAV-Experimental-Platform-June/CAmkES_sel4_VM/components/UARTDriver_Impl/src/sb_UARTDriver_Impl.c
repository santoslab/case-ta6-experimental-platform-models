...
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_AirVehicleState_WPM_Send(STACK_FRAME 
  art_DataContent d) {
  sb_AirVehicleState_WPM_enqueue(d);
}

// send AirVehicleState_UXAS: Out EventDataPort CMASI__AirVehicleState_i
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_AirVehicleState_UXAS_Send(STACK_FRAME 
  art_DataContent d) {
  sb_AirVehicleState_UXAS_enqueue(d);
}

// FIXME: dummy implementation for unconnected outgoing port
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_send_data_Send(STACK_FRAME 
  art_DataContent d) {
  // FIXME: dummy implementation
}

// is_empty MissionCommand: In EventDataPort
B hamr_Drivers_UARTDriver_Impl_seL4Nix_MissionCommand_IsEmpty(STACK_FRAME_ONLY) {
  return sb_MissionCommand_is_empty();
}

// receive MissionCommand: In EventDataPort union_art_DataContent
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_MissionCommand_Receive(STACK_FRAME
...
B hamr_Drivers_UARTDriver_Impl_seL4Nix_recv_data_IsEmpty(STACK_FRAME_ONLY) {
  return T;
}

// FIXME: dummy implementation for unconnected incoming port
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_recv_data_Receive(STACK_FRAME
  Option_8E9F45 result) {
  // FIXME: dummy implementation
...
void pre_init(void) {
  ...
  hamr_Drivers_UARTDriver_Impl_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  hamr_Drivers_UARTDriver_Impl_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    hamr_Drivers_UARTDriver_Impl_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
